...
export default function ProfessionalsPage() {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Directory Grid */}
      {filtered.length === 0 ? (
        <div className="bg-white border border-stone-200 rounded-3xl p-12 text-center space-y-4 max-w-xl mx-auto shadow-sm">
          <div className="w-16 h-16 bg-emerald-50 text-emerald-600 rounded-2xl flex items-center justify-center mx-auto">
            <ShieldCheck className="w-8 h-8" />
          </div>
          <h3 className="text-xl font-bold text-stone-900">
            {professionals.length === 0 ? "No Verified Advocates Listed Yet" : "No Practitioners Found"}
          </h3>
          <p className="text-sm text-stone-600 leading-relaxed">
            {professionals.length === 0
              ? "All legal profiles on Ukil require Bangladesh Bar Council enrollment and manual KYC verification. Are you a licensed Advocate or Barrister? Register today to connect with citizens seeking legal assistance."
              : `No legal professionals matched "${searchQuery}". Please try searching for a different specialty or location.`}
          </p>
          {professionals.length === 0 ? (
            <Link
              href="/signup/lawyer"
              className="inline-flex items-center gap-2 bg-brand-coral hover:bg-brand-hover text-white text-sm font-bold px-6 py-3 rounded-xl shadow-coral transition-colors"
            >
